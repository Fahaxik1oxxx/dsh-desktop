...
- 主进程 spawn 本地 node 服务器（`node apps/cli/lib/bin.js web --no-open`，默认 127.0.0.1:3080），从启动输出解析本进程带 token 的 URL 后只在 Electron 窗口加载；`--no-open` 关掉上游默认的系统浏览器跳转。启动前预检端口占用，被别的程序占了会直接报明确原因。右上角 最小化/最大化/关闭 为注入页面的自绘按钮（透明融于界面，无 OS 原生按钮）。
